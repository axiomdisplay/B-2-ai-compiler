#pragma once
// B-2 passes tests - minimal self-registration harness (same discipline).
//
// Mirrors tests/frontend/TestHarness.h exactly (same B2_TEST semantics,
// same registry/failure model) so both suites share one discipline: tests
// self-register at static-init time, TestMain runs the registry in
...
// binary; every test here is prefixed gb_.
