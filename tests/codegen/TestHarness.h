#pragma once
// B-2 codegen (T1 instantiation) tests - minimal self-registration harness.
//
// Mirrors tests/interp/TestHarness.h exactly (same B2_TEST semantics, same
...
// binary; every test here is prefixed codegen_.
//
// This suite is the machine-code oracle for Tier 1 (docs/codegen_contract.md;
// Rule 36 differential form): every assertion encodes the archive contract
// (Stencil Rules 1-5), the instantiator's refusals (Amendment A), the
// activation/ABI static_asserts, and - the heart - byte-exact differential
// execution of the interpreter corpus + the frontend-lowered corpus on
// T1 machine code against the T0 golden outputs.
