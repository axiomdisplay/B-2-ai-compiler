...
- ICDG Phase 2 landed (the T2 side: ingestion + GuardInline; MSG-20260901-005): `passes::DispatchProfile` (the T0 dispatch-profile snapshot, site key `(caller MethodId, call pc)` recovered from the call's FrameState; runtime class ids bridge to the resolver TypeId space by NAME) + the shared converter `compiler/passes/tools/DispatchProfileSnapshot.h` + `b2graph --pgo` (T0 training run -> snapshot -> engine, decision lines carry the profile numbers). A monomorphic `CallVirtual`/`CallInterface` site inlines behind a `TypeProfile` guard (`InstanceOf` condition, deopt at the call pc re-executes the invoke - the guard runs before any body effect), with complete Rule 122 SpecMeta (TypeMonomorphic/PGO/confidence/cost) + a `ClassHierarchy` invalidation dependency (Rule 42; the C2 shape). Bimorphic/polymorphic/megamorphic/low-confidence/Object-profiled sites refuse with structured reasons; the passes LIBRARY stays interp-free (the tool and the test harness are the interp-linking integrators). Zero-regression pinned: no profile attached = virtual sites not considered, v1-identical bytes. 20 new tests (suite 49) + a profile-attached corpus-sweep variant; full matrix green (Law-36 19/19, `--pgo -O` corpus 19/19, ASan/UBSan clean). Found on the way: the IR verifier's memory-chain walk false-positives on any memory producer inside a loop (MSG-20260901-004) and a passes-side builder defect on interface-call type probing (fixed in-tree). Contract: `docs/inlining.md` section 9.
- IR verifier/type batch landed (MSG-009, MSG-002, MSG-004 all closed): `resultTypeOf` now matches Node.h/ir_spec for the five mis-grouped rows (comparisons `CmpL/CmpFl/CmpFg/CmpDl/CmpDg` -> Int, `I2L` -> Long; operand-side checks unchanged, zero golden churn); the memory-chain walk treats an on-path revisit of a **LoopBegin-backed** memory Phi as the legal loop backedge closure (Region-backed phis and non-Phi revisits stay cycle errors — a real cycle through a forward merge is still caught); `checkFrameStates` requires every caller-chain target to resolve to a live FrameState node (the side-table liveness the deoptimizer depends on; ir_spec 7 amended). The MSG-004 unblock is proven end to end: the canonical dispatch-profile loop program (`main { while (i<3) sum += obj.bump(5) }`) now trains in T0, snapshots 3/3 mono, and GUARD-INLINES the call inside the loop (test `il_guard_call_in_loop_end_to_end`; `b2graph --pgo -O` demo) — the most common real-world inline shape is back on the menu. The `L2I(I2L(x))` round-trip identity is unblocked and consumed (`ident_widen_narrow_round_trip`). 11 new tests (ir 90, passes 146, inline 50); full matrix 823/823, Law-36 19/19, corpus gates 19/19, ASan/UBSan clean. MSG-009's tombstone-removal request: decision deferred to a dedicated RFC (recorded in the close). Contract: `docs/ir_spec.md` section 7.
- CM-PEA v1 landed (the Part XVIII PEA pass, keys 65/66/67/69 - one engine, four stage keys): `compiler/passes/src/Escape.cpp` + `b2graph --pea`. Every allocation is classified on the height-6 escape lattice (special_passes.md 1.1) by one monotone pass over its live uses; dispositions are the four PEA Rule options - SCALARIZED (no-escape: loads forward to the nearest same-field mem-chain store, virtual stores splice out of both chains, IsNull folds, the allocation dies), MATERIALIZED (arg/field/return escape: the first escape use becomes `Materialize [ctrl, mem, vobj]` wired into BOTH chains, pre-escape reads still forward, every later use and FrameState locals edge reads the real reference via Rule 14 auto-rebase), and REJECTED (identity/exception/fs/phi observability, cost gates, kill switches - each with a catalog reason). The straight-segment scope gate (equal branch-projection sets, no merge crossings) is the soundness core that makes chain order == program order without a solver; cost gates per Rule 45 (kPeaMaxFields/kPeaMaxArrayElems/kPeaMaxAllocsPerGraph, Law 23). Two requests to the ir team rode the landing: MSG-20260901-006 (appendFrameStateVobj - the deopt-materialization channel that unlocks post-inline scalarization) and MSG-20260901-007 (memory-chain DFS step belt false positive on long chains). 40 tests; full matrix 10/10 suites (863 individual), ASan/UBSan clean, corpus gates green with the pipeline extension (`pea` between GVN and the final DCE). Contract: `docs/pass_contracts.md` section 12 + `docs/special_passes.md` section 1 status block.
- CM-PEA v1.1 landed (the fs-escape listing - the ir team's appendFrameStateVobj and DFS-belt fixes consumed): a NoEscape allocation referenced by deopt snapshots no longer rejects with `fs-deopt-ref` - it SCALARIZES, and every referencing FrameState gets a per-instant `VirtualObjectState` (the field state resolved by a backward chain lookup, NOT id order: inlined-body stores carry higher ids than the call-site fss they follow in program order) with the fs's slot edges rebased onto it and the desc listing it via `appendFrameStateVobj` (the deopt-materialization channel). IDENTITY is the soundness core: every fs of the allocation inside one live deopt chain is reconstructed at the same instant - the same object in several frames - so they share ONE vobj (union-find over co-occurring fss, with a serving-deopt-points agreement check; disagreement rejects `fs-multi-deopt`); deopt-UNREACHABLE userless snapshots (the post-inline call-site fss after nullfold+DCE) are dead metadata sharing one final-state vobj. The fields.rbc post-inline e2e reaches ZERO allocations - the inline-engine + PEA end state the tiers were built for. The MSG-20260901-007 fix also restored the instance-field cost-gate test to its real shape (the v1 array workaround retired). 49 tests (9 new); full matrix 10/10 (878 individual), ASan/UBSan clean, the 19-program corpus through `--inline --pea -O` 19/19. Contract: `docs/pass_contracts.md` section 12 (the fs-escape listing block).
- SCCP v1 landed (the sparse conditional constant propagation pass, key 38 - one engine covering the charter's scalar rows 37/38/39, exactly like GVN covers CSE 36): `compiler/passes/src/SCCP.cpp` + the registry row + the pipeline slot right after the simplify sweep. Wegman-Zadeck adapted to the sea of nodes: an optimistic Top/Const/Bot value lattice where pure values evaluate on operand descent (they float - no schedule, no dominator walk), phis meet over EXECUTABLE region inputs only (a not-yet-firing backedge contributes Top - the optimism that resolves loop-invariant phis; meet(0, phi) stabilizes, meet(0, phi+1) refutes), and If/Guard conditions decide edge executability through the LATTICE - the constants constfold's literal-only view cannot see (a phi merging 5 and 10-5, a branch on EqI(1,1) before any fold ran). The completion rule is the soundness bolt: at fixpoint an unresolved-but-USED Top value is forced overdefined and re-propagated (floating values have no block, so "unreachable" is not an answer) - a hand-buildable phi with an unresolvable executable edge collapses to Bot instead of claiming. Rewrites are minimal by design: interned constant nodes replace the proven values (id order, cycle-safe phi typing belt, FrameState auto-update per Rule 14); CONTROL stays branchnorm's business, and the same round's nullfold/branchnorm/cfs/dce consume SCCP's new constants (a decided-by-propagation branch folds and sweeps in ONE round - test-pinned). The fold semantics are NOT a second source: SCCP calls the same evalBinOp/evalUnaryOp table constfold uses (lifted to the shared PassInternal.h seam). The registry passed 16 rows, so PassConfig::enabledBits widened to 32 (the accessors are the API; bit i mirrors registry row i). 21 new tests (passes suite 167); full matrix 10/10 (899 individual), ASan/UBSan clean (the sanitizer caught a real apply-loop OOB - the interned constants grow nodeCount mid-apply, the candidate bound is frozen at entry), the 19-program corpus through `--inline --pea -O` 19/19, the fields.rbc zero-allocation flagship intact, and the `--pgo -O loop_call_demo.rbc` demo now shows sccp=6 (the guard-inline path feeds phi-meet constants). Contract: `docs/pass_contracts.md` section 13.
- Next: the GuardInline growth (bimorphic two-target guard, charter item 26; CHA subclass proofs once hierarchy analysis exists; the CmpL long zero-guard switch - optional cleanup); the CM-PEA growth paths (phi-of-stores field SSA, dynamic-index speculation, EscapeSummary reuse, per-deopt caller-frame copies for multi-instant chains); the remaining mid-suite scalar passes (CSE is structurally covered by GVN for identical keys; PRE needs the memory-alias machinery); the loop-pass family over the now-verifiable call-in-loop shapes; GCR Phase 1 (region allocator, TLAB, young scavenge) per `docs/gc.md` - the T1 activation record is the GC's frame-scanning contract (every slot is a tagged Value); string-concatenation builtins RFC (interpreter) to lift the lowering's top refusal; the full-Java frontend agenda (multi-class compilation, inheritance/interfaces across user classes, string concatenation, autoboxing, generics/lambdas/records lowering - the parse-only corpus vs the lowered subset gap).
