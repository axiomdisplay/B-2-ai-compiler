...
- Next: T2 IR construction kickoff (CM-PEA designs per `docs/special_passes.md`; ICDG per `docs/icdg.md` - the T0 interpreter's profile counters are its Phase 1 dispatch profiler, and the T1 helper seam is the runtime-call ABI the T2 backend reuses); GCR Phase 1 (region allocator, TLAB, young scavenge) per `docs/gc.md` - the T1 activation record is the GC's frame-scanning contract (every slot is a tagged Value); string-concatenation builtins RFC (interpreter) to lift the lowering's top refusal.
