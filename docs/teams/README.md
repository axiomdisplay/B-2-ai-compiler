...
- Next: ICDG Phase 1 dispatch profiling (call-site receiver/target counters in T0) to unlock GuardInline (TypeProfile guards + Rule 122 metadata); CM-PEA per `docs/special_passes.md`; the mid-suite scalar passes (CSE/PRE/SCCP) once MSG-009 lands; GCR Phase 1 (region allocator, TLAB, young scavenge) per `docs/gc.md` - the T1 activation record is the GC's frame-scanning contract (every slot is a tagged Value); string-concatenation builtins RFC (interpreter) to lift the lowering's top refusal.
