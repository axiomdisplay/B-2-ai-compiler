...
- Next: the GuardInline growth (bimorphic two-target guard, charter item 26; CHA subclass proofs once hierarchy analysis exists; the CmpL long zero-guard switch - optional cleanup); the CM-PEA growth paths (fs-vobj listing once MSG-006 lands, phi-of-stores field SSA, dynamic-index speculation, EscapeSummary reuse); the mid-suite scalar passes (CSE/PRE/SCCP - the MSG-009 type blocker is lifted); the loop-pass family over the now-verifiable call-in-loop shapes; GCR Phase 1 (region allocator, TLAB, young scavenge) per `docs/gc.md` - the T1 activation record is the GC's frame-scanning contract (every slot is a tagged Value); string-concatenation builtins RFC (interpreter) to lift the lowering's top refusal.
