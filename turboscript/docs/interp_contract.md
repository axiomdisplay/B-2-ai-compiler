...
| Rule 23 (no magic constants) | Every threshold (call depth, megamorphic limit, smi range, string limit) is a named `constexpr` in `ts_limits.h`. |
...
## 3. Value Model (strict-mode semantics, v0.1)

- Tagged values: Undefined, Null, Boolean, Smi (int32), HeapNumber (double),
  String (UTF-16), Object, BigInt. (Symbol reserved; not user-visible yet.)
...
- Objects: shape-based property storage (transition tree per isolate),
  ...
  semantics (TypeError on frozen/non-writable stores). Dictionary mode and
  arrays are deferred (bytecode_spec.md Section 11).
...
- Corpus tests are self-contained `.tsbc` + `.out` pairs under
  ...
  and Rule 90 stack-overflow behavior.
...
## 9. Out of Scope for v0.1 (tracked in bytecode_spec.md Section 11)

Arrays, Proxy, generators/async, sloppy mode, GC, user symbols, eval.
Each is owned and expiry-dated there per Rule 143. Nothing in this list is
silently degradable: opcodes or semantics that depend on them do not exist
in v0.1 bytecode.
